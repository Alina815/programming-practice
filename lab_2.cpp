...
public:
    ...
    double calculateCost() const {
        ...
        if (abs(duration) > numeric_limits<double>::max() / abs(tariff->getPricePerMinute())) {
            throw overflow_error("Стоимость звонков слишком велика.");
        }
...
public:
    ...
    double getTotalCost() const {
        ...
        for (const auto& call : calls) {
            if (abs(total) > numeric_limits<double>::max() - abs(call.calculateCost())) {
                throw overflow_error("Стоимость звонков слишком велика.");
            }
...
public:
    ...
    double calculateAllCallsTotal() const {
        ...
        for (const auto& customer : customers) {
            try {
                if (abs(total) > numeric_limits<double>::max() - abs(customer.getTotalCost())) {
                    throw overflow_error("Стоимость звонков слишком велика.");
                }
