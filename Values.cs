...
namespace lab_5
{
    public partial class Values : Form
    {
        ...
        private void InitializeList()
        {
            ...
            var btnDeleteTariff = new Button
            {
                Location = new System.Drawing.Point(653, 350),
                Size = new System.Drawing.Size(300, 30),
                Text = "Удалить тариф"
            };
            btnDeleteTariff.Click += btnDeleteTariff_Click;
            SetListToGrid();
            ClientSize = new Size(973, 400);
            Controls.AddRange(new Control[] {dgvTariffList, btnClose, btnLoadFromFile, btnSaveToFile,
            btnSortByPrice, btnSortByCity1, btnSortByCity2, btnDeleteTariff});
            Text = "Список тарифов";
        }
        ...
        private void btnDeleteTariff_Click(object sender, EventArgs e)
        {
            string input = Microsoft.VisualBasic.Interaction.InputBox(
                "Введите номер удаляемого тарифа:",
                "Удаление тарифа"
            );
            ...
            if (index < 1 || index > TariffList.Count)
            {
                MessageBox.Show($"Индекс должен быть в диапазоне от 1 до {TariffList.Count}", "ОШИБКА", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            TariffList.RemoveAt(index - 1);
            Ate.SetLists(TariffList);
            SetListToGrid();
            MessageBox.Show("Тариф успешно удалён!", "УСПЕХ", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
    }
}